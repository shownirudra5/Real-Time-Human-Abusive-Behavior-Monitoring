...
# Metrical Factors (3D Units to Meters)
FACTOR_X = 8.353 / 1.86  
FACTOR_Y = 27.0 / 6.136  

# PLY Aesthetics
TRAJECTORY_COLOR = (255, 100, 0) # Orange
DISC_RADIUS      = 0.06
DISC_SAMPLES     = 24
INTERP_STEPS     = 12
Z_LIFT           = 0.05
...
# ============================================================
# 1. CAMERA CALIBRATION & GROUND PLANE (From your reference)
# ============================================================
pts_3d_all = np.array([
...
cam_ref  = (-R_ref.T @ tvec_ref).flatten()

# Ground Plane SVD
floor_mask = pts_3d_all[:,2] < 1.2
...
def ray_ground(u, v):
    ...
    ray_wld = R_ref.T @ ray_cam
    denom = np.dot(n_floor, ray_wld)
    if abs(denom) < 1e-6: return None
    t = -(np.dot(n_floor, cam_ref) + d_floor) / denom
    return cam_ref + t*ray_wld + n_floor*Z_LIFT
...
print("Processing... Tracking distance and building 3D path.")
    ...
    if res[0].boxes.id is not None:
        ...
        if primary_id in ids:
            ...
            if (y2 - y1) / (x2 - x1) > 1.8:
                ...
                p3d = ray_ground(u_foot, v_foot)
                
                if p3d is not None:
                    if len(path_3d) > 0:
                        dx_m = abs(p3d[0] - path_3d[-1][0]) / FACTOR_X
                        dy_m = abs(p3d[1] - path_3d[-1][1]) / FACTOR_Y
                        step = np.sqrt(dx_m**2 + dy_m**2)
            ...
            cv2.circle(frame, (int((x1+x2)/2), int(y2)), 6, (0, 165, 255), -1)
            
            status = "FEET DETECTED" if feet_detected else "PAUSED - NO FEET"
            ...
            cv2.putText(frame, status, (30, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
...
# ============================================================
# 4. EXPORT PLY TRAJECTORY (The "Discs" Path)
# ============================================================
print(f"Total points: {len(path_3d)}. Building 3D Trajectory...")
all_verts = []
for i in range(len(path_3d)):
    # Add main disc
    all_verts.extend(make_disc(path_3d[i], n_floor, DISC_RADIUS, DISC_SAMPLES, TRAJECTORY_COLOR))
    # Interpolate between points for smooth visual path
    if i < len(path_3d)-1:
        for s in range(1, INTERP_STEPS):
            a = s/INTERP_STEPS
            p = (1-a)*path_3d[i] + a*path_3d[i+1]
            all_verts.extend(make_disc(p, n_floor, DISC_RADIUS*0.4, DISC_SAMPLES//2, TRAJECTORY_COLOR))
...
with open(OUTPUT_PLY, 'w') as f:
    ...
    for v in all_verts:
        f.write(f"{v[0]:.6f} {v[1]:.6f} {v[2]:.6f} {int(v[3])} {int(v[4])} {int(v[5])}\n")

cap.release()
out_video.release()
csv_file.close()
cv2.destroyAllWindows()
print(f"Success! PLY trajectory saved to: {OUTPUT_PLY}")